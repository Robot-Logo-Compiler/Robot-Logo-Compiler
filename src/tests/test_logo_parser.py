...
class TestParser(unittest.TestCase):
    ...
    def test_sipmle_string_test(self):
        tokens = [("KEYWORD", "show"), ("PARAMETER", '"Moikka')]
        tree = parse(tokens)
        self.assertEqual('"Moikka', tree.root.children[0].child.value)

    def test_simple_binary_operation_works_with_a_variable(self):
        tokens = [('FUNCTION', 'sqrt'), ('PARAMETER', "1"), ("BIN_OP", "plus"), ('VARIABLE', "x")]
        tree = parse(tokens)
