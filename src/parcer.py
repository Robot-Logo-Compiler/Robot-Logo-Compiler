from logo_parser_mop import KeywordNode, ParserTree, CodeNode, ParameterNode, BinaryOperationNode


class Tokens:
    """A class that manages the token list, providing semi-global variables
    for the parcing funktions
    """
...
def parce(tokens):
    """Function that creates a parce tree for an input token list
    Input:
        tokens: token list containing tokens as tuples
    Return:
        ParcerTree-class for the given token list
    """
